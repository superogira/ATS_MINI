...
void setup()
{
  ...
  server.on("/wificonnect", webWifiConnect);
...
void handle_OnConnect() {
  server.send(200, "text/html", WifiConnection()); 
}
...
String WifiConnection(){
  ...
  ptr +="<html>";
  ptr +="<head>";
  ptr +="<title>WiFi Connection</title>";
  ...
  ptr +="</head>";
  ptr +="<body>";
  ptr +="<h1>WiFi Connection</h1>";
  ptr +="<div class='container'>";
  
  ptr +="<br><div id='setWifi'><form action=\"/wificonnect\" method=\"POST\"><label for=\"wifissid\">WiFi SSID : </label><input type=\"text\" name=\"setWifiSSID\" id=\"setwifissid\"><br><label for=\"wifipassword\">WiFi Password : </label><input type=\"password\" name=\"setWifiPassword\" id=\"setwifipassword\"><br><br><input type='submit' value='Connect WiFi (Only 2.4GHz)'></form></div><br>";
  ptr +="<br><div id='setUTCoffset'><form action=\"/wificonnect\" method=\"POST\"><label for=\"utcoffset\">UTC Offset in seconds : </label><input type=\"number\" min='-43200' max='43200' name=\"setUTCoffsetSeconds\" id=\"setutcoffsetseconds\"> <input type='submit' value='Set UTC Offset'></form></div><br>";
  ptr +="<br><div id='setTime'><form action=\"/wificonnect\" method=\"POST\"><label for=\"hours\">Hours : </label><input type=\"number\" min='0' max='24' name=\"setHours\" id=\"sethours\" size=\"2\"> <label for=\"minutes\">Minutes : </label><input type=\"number\" min='0' max='60' name=\"setMinutes\" id=\"setminutes\" size=\"2\"> <input type='submit' value='Manual set time (if no internet)'></form></div><br>";

  ptr +="<br><br><br><div><form action=\"/radio\" method=\"POST\"><input type='submit' value='Skip'></form>";
...
String SendHTML(){
  ...
  ptr +="<br><div id='setFreq'></div>";
  ptr +="<br><div id='setVolume'><FORM METHOD='POST'action='/setvolume'><label for='Volume'>0 - 63 :</label><input type='number' min='0' max='63' name='setVolume'><input type='submit' value='Set Volume'></form></div><br>";
  //ptr +="<br><div id='setWifi'><form action=\"/wificonnect\" method=\"POST\"><label for=\"wifissid\">WiFi SSID :</label><input type=\"text\" name=\"setWifiSSID\" id=\"setwifissid\"><br><label for=\"wifipassword\">WiFi Password :</label><input type=\"password\" name=\"setWifiPassword\" id=\"setwifipassword\"><br><br><input type='submit' value='Connect WiFi (Only 2.4GHz)'></form></div><br>";
  ...
  ptr +="<br><br><div><form action=\"/\" method=\"POST\"><input type='submit' value='WiFi Connection / Time Setup'></form>";
  ...
  ptr +="</script>";
  ptr +="<script type=\"text/javascript\">";
  ptr +="var smeterGuage = 0; var lastMode = \"\"; $(function(){setInterval(function(){var getData=$.ajax({url:\"./data\",async:true,success:function(getData){const data = getData.split(\" \");$(\"div#Mode\").html(data[0]);$(\"div#Frequency\").html(data[1]+\"<span class='superscript' id='Tail'></span>\");$(\"span#Tail\").html(data[2]);$(\"div#Volume\").html(data[4]);smeterGuage = data[3];if (data[0] == \"FM\" && lastMode != \"FM\") {lastMode = \"FM\"; $(\"div#setFreq\").html(\"<FORM METHOD='POST'action='/setfrequency'><label for='frequency'>6400 - 10800 :</label><input type='number' min='6400' max='10800' name='setFrequency'><input type='submit' value='Set Frequency'></form>\");} else if (data[0] != \"FM\" && lastMode != \"OTHER\") {lastMode = \"OTHER\";$(\"div#setFreq\").html(\"<FORM METHOD='POST'action='/setfrequency'><label for='frequency'>150 - 30000 :</label><input type='number' min='150' max='30000' name='setFrequency'><input type='submit' value='Set Frequency'></form> <br> <FORM METHOD='POST'action='/setfrequency'><label for='bfo'>0 - 999 :</label><input type='number' min='0' max='999' name='setBFO'><input type='submit' value='Set BFO'></form>\");} else {}}}).responseText;},";
  ptr +=ajaxInterval;
  ptr +=");});";
  ptr +="</script>";
