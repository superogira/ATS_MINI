...
void setup()
{
  ...
  server.on("/rssichart", HTTP_POST, [](AsyncWebServerRequest *request){
    ...
    request->send(200, "text/html", webRSSIChart());
  });
  server.on("/rssichart", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/html", webRSSIChart());
  });
...
void webRadio(AsyncWebServerRequest *respond) {
  //respond->setContentLength(65535);
